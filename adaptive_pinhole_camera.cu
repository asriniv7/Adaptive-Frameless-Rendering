...
rtDeclareVariable(unsigned int,	 tile_size, , );
rtDeclareVariable(float,			 number_of_parent_tiles, ,); 

rtBuffer<uchar4, 2>              output_buffer;
rtBuffer<uchar4, 2>				 color_buffer; //Stores the pixel's color
...
rtBuffer<unsigned int,1>		leaf_tile_sizes; 
...
RT_PROGRAM void pinhole_camera()
{
	...
	unsigned int new_seed = seed;
	float uu = rnd(new_seed)*512.0f;
	float vv = rnd(new_seed)*512.0f;
	rnd_seeds[launch_index] = new_seed;

	uint2 new_index = make_uint2(uu, vv);
	float3 result = trace(make_float2(new_index));
	output_buffer[new_index] = make_color(result);
	color_buffer[new_index] = make_color(result);
}

RT_PROGRAM void pinhole_camera_all()
{
	float3 result = trace(make_float2(launch_index));
	output_buffer[launch_index] = make_color(result);
	color_buffer[launch_index] = make_color(result); 
}

RT_PROGRAM void pinhole_camera_old()
{
